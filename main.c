...
//#pragma message("---- PROBE: before nrfx.h")
//#include "nrfx.h"
//#pragma message("---- PROBE: after nrfx.h")
//#pragma message("VAL NRFX_TWIM_ENABLED="  STR(NRFX_TWIM_ENABLED))
//#pragma message("VAL NRFX_TWIM0_ENABLED=" STR(NRFX_TWIM0_ENABLED))
//#define STR2(x) #x
//#define STR(x) STR2(x)

static void very_early_probe(void) {
    SEGGER_RTT_WriteString(0, "\r\n=== BOOT ===\r\n");
    nrf_gpio_cfg_output(PIN_LED);   // PIN_LED -> P0.18
    for (int i = 0; i < 6; i++) {
...
static void startup_led_seyi(void)
{
    // LED pini çıkış olarak ayarla
    nrf_gpio_cfg_output(PIN_LED);
...
static void idle_state_handle(void)
{
    app_sched_execute();                 // scheduler kuyruğunu boşalt
    (void)NRF_LOG_PROCESS();             // DEFERRED logları pompa et
    nrf_pwr_mgmt_run();                  // SD varsa sd_app_evt_wait() ile uyutur
}
...
very_early_probe();
    ...
    //APP_ERROR_CHECK(app_timer_start(m_fds_kick_tmr, APP_TIMER_TICKS(2500), NULL));
     ...
     for (;;)
{
    // 1) Scheduler + Log + Power management
    idle_state_handle();

    // 2) Senin mevcut FDS GC mantığın
