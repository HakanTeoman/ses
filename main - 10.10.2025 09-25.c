...
static void twi_acquire(void)
{
    ...
    if (nrf_drv_twi_init(&m_twi, &cfg, NULL, NULL) == NRF_SUCCESS) {
...
static bool rtc_write_regs(uint8_t reg, const uint8_t *buf, size_t len)
{
    ...
    tx[0] = reg;
    memcpy(&tx[1], buf, len);
    ret_code_t err = nrf_drv_twi_tx(&m_twi, RTC_I2C_ADDR, tx, (uint8_t)(len+1), false);
    twi_release();
...
{
    twi_acquire();
    ret_code_t err = nrf_drv_twi_tx(&m_twi, RTC_I2C_ADDR, &reg, 1, true);
    if (err != NRF_SUCCESS) { twi_release(); return false; }
    err = nrf_drv_twi_rx(&m_twi, RTC_I2C_ADDR, buf, (uint8_t)len);
    twi_release();
    return (err == NRF_SUCCESS);
}
...
static inline void battery_path_enable(bool en)
{
    ...
    nrf_gpio_pin_write(PIN_DCDC_EN, en ? 1 : 0);
#else
    nrf_gpio_pin_write(PIN_DCDC_EN, en ? 0 : 1);
#endif
}
static inline void dcdc_quick_discharge(void)
{
#if BAT_PATH_ACTIVE_HIGH
    nrf_gpio_pin_write(PIN_DCDC_EN, 0);
#else
    nrf_gpio_pin_write(PIN_DCDC_EN, 1);
#endif
    nrf_gpio_cfg_input(PIN_DCDC_EN, NRF_GPIO_PIN_PULLDOWN);
    nrf_delay_us(60);
    nrf_gpio_cfg_output(PIN_DCDC_EN);
#if BAT_PATH_ACTIVE_HIGH
    nrf_gpio_pin_write(PIN_DCDC_EN, 0);
#else
    nrf_gpio_pin_write(PIN_DCDC_EN, 1);
...
static void gpio_defaults_init(void)
{
    ...
    nrf_gpio_cfg_output(PIN_VGLB_IN1); nrf_gpio_pin_write(PIN_VGLB_IN1, 0);
    nrf_gpio_cfg_output(PIN_VGLB_IN2); nrf_gpio_pin_write(PIN_VGLB_IN2, 0);

    nrf_gpio_cfg_output(PIN_DCDC_EN);  nrf_gpio_pin_write(PIN_DCDC_EN, 0);
